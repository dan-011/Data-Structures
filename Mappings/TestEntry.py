...
class TempMap:
    ...
    def __getitem__(self, pos):
        ...
        if pos not in self: raise KeyError("This position does not exist")
            ...
            maxindex = self.find_max(pos)
            minindex = self.find_min(pos)
            maxim = bucket[maxindex].temp
            minim = bucket[minindex].temp
            return (minim, maxim)
    ...
    def add_report(self, pos, temp):
        ...
        if pos not in self:
            ...
            max_index = self.find_max(pos)
            min_index = self.find_min(pos)
            if max_index == min_index:
                self.L[location].append(entry)
                self.len += 1
            else:
                if self.L[location][max_index] < entry:
                    self.L[location][max_index] = entry
                if self.L[location][min_index] > entry:
                    self.L[location][min_index] = entry
            if len(self) > self.buckets*2: self.rehash()
    def find_max(self, pos):
        location = self.hash(pos)
        bucket = self.L[location]
        entries = []
        for i in range(len(bucket)):
            if bucket[i].rounded_position() == (round(pos[0],1),round(pos[1],1)):
                entries.append(i)
        if len(entries) == 1: return entries[0]
        else:
            item1 = self.L[location][entries[0]]
            item2 = self.L[location][entries[1]]
            if item1 > item2: return entries[0]
            else: return entries[1]
    def find_min(self, pos):
        location = self.hash(pos)
        bucket = self.L[location]
        entries = []
        for i in range(len(self.L[location])):
            if bucket[i].rounded_position() == (round(pos[0],1),round(pos[1],1)):
                entries.append(i)
        if len(entries) == 1: return entries[0]
        else:
